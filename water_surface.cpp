...
void water_surface::update(float dt) {
    for (int i = 0; i < this->width; i++) {
        for (int j = 0; j < this->height; j++) {
            ...
            this->v[i][j] += f * dt;    //感觉该函数没什么物理意义，重要的要有理有据地给一个初速度
    ...
    for (int i = 3; i < this->width; i += 3) {
        for (int j = 3; j < this->height; j += 3) {
            ...
            for (int k = 0; k < 9; k++) {
                ...
                //此时就是将点转换为了网格,我比较不理解的是这个网格的转换跟教程上是不一样的
            ...
            //这个公式看不明白原理也没有讲，略过，被拷打到也没有办法
            //比较疑惑的是z不管怎么说一定会有大于的情况，那么不会一直增加吗？
            //我是疑惑不解的
            vec3 p = points[0];
            for (int k = 1; k < 9; k++) {
                vec3 p0 = points[k];
