...
water_surface::water_surface() {
    ...
    for (int i = 0; i < this->width; i++) {
        for (int j = 0; j < this->height; j++) {
            //x和y的作用是什么甚至说删除下面两句话都没有任何影响吧？而且这个奇怪的形式似乎是一个公式后面也用得上？
            float x = -3 + 6 * (1 - (i / (float) this->width));
            float y = -3 + 6 * (1 - (j / (float) this->height));

            //特定的ij的u值是0.7，其他的是0，速度一直是零，为何ij的值初始化是有区分的？
...
void water_surface::update(float dt) {
    for (int i = 0; i < this->width; i++) {
        for (int j = 0; j < this->height; j++) {
            //v1，v2，v3，v4的作用是什么？保存了位置向量？
            float v1, v2, v3, v4;

            //v1，v2，v3,v4保存的向量有点类似于四个方向了，涉及到网格了吗？
            if (i == 0) {
                v1 =  this->u[i][j];
            }
            else {
            ...
            //v为何是+=差值乘以时间？0.995是什么意思？为什么会有这么一个数字？
            //u_new就是新的位置，非常简单易懂的物理速度方程
    ...
    //但是control_point_heights是为了什么？是不是有一点多此一举？
    for (int i = 0; i < this->width; i++) {
        for (int j = 0; j < this->height; j++) {
    ...
    //Δx和Δy常量如果我理解的没错的话，插值不是要靠算的吗？怎么这里直接就给了？
